<script lang="ts">
   ...
   *
   * Modifications from the upstream SVG:
   *   - `stroke="black"` → `stroke="currentColor"` so the logo inherits the parent's
   *     text color, matching the Lucide icon API used elsewhere in the docs site.
   *   - clipPath id renamed to `mcp-logo-clip` to avoid collisions with other inlined SVGs.
   *   - `size` prop matching Lucide's API (number, defaults to 18).
   */
  let {
    size = 18,
    class: className = ''
