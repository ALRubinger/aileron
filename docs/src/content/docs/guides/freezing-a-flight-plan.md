...
- QEMU emulators for cross-architecture builds. The dedicated builder already bundles these, so most hosts need nothing here. If freeze still reports a missing emulator, register them once with `docker run --privileged --rm tonistiigi/binfmt --install all`; freeze aborts with this exact command in the error rather than silently producing a single-architecture pin.
