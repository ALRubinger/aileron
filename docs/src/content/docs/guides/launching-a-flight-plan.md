...
The walk visits each input in declaration order and prompts once per promptable literal input. Each prompt shows the input name, its declared description, a `[required]` or `[required · has default]` marker, a declared `example` value when the input carries one, and any enum or pattern constraint hint so you know the accepted shape. Every literal input is required. A `[required · has default]` input carries a default, so it also shows that default and accepts it when you press Enter. Pressing Enter leaves the declared default to resolve natively, exactly as `--accept-defaults` would, so a number default stays a number whether you accept it interactively or launch with defaults. The walk records no override for an accepted default, so an interactive launch and a defaulted launch resolve the identical value and type.

An input the manifest marks `prompt: false` is advanced or non-interactive, so the walk skips it and its declared default applies silently. The walk renders an informational line noting the skip. The input stays overridable with `--input name=value`, and an explicit override is shown as already set. Marking a required input with no default `prompt: false` leaves it with no value to apply, so that launch fails at resolution unless the value is supplied with `--input`.
