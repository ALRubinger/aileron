...
The walk visits each input in declaration order and prompts once per promptable literal input. Each prompt shows the input name, its declared description, a `[required]` or `[optional]` marker, a declared `example` value when the input carries one, and any enum or pattern constraint hint so you know the accepted shape. An optional input also shows its current default and accepts it when you press Enter. Pressing Enter leaves the declared default to resolve natively, exactly as `--accept-defaults` would, so a number default stays a number whether you accept it interactively or launch with defaults. The walk records no override for an accepted default, so an interactive launch and a defaulted launch resolve the identical value and type.
