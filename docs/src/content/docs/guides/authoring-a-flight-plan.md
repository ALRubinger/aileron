...
A `command` element may embed a `{{ inputs.<name> }}` token to parameterize the argv from a declared input. The runtime substitutes the token with the input's resolved value at launch. So `--region={{ inputs.region }}` execs as `--region=us-east-1`. The substitution stays inside the element and never introduces a shell or splits the argv, so the no-shell guarantee holds. The referenced input must declare a `constraint` (an `enum` or a `pattern`). Freeze rejects a token that references an unconstrained input, because an unbounded value in a signed command position is an injection surface. Bound the value with a constraint and the sealed command stays within the shape you signed. A token referencing an undeclared input, or a malformed token, is rejected at freeze and again at launch load.

A `trustContract.hosts` entry may embed a `{{ inputs.<name> }}` token the same way, so a region-parameterized endpoint like `athena.{{ inputs.aws_region }}.amazonaws.com` is one host entry rather than one entry per region. The referenced input must declare a `constraint`, and freeze rejects a host token that references an unconstrained input, because an unbounded value in a signed host position could steer the step's egress to an attacker-chosen host. The sealed host template is instantiated from the resolved input at launch, and the daemon proxy matches the concrete host by exact string, so a sibling region the constraint does not admit is denied. A host token referencing an undeclared input, or a malformed token, is rejected at freeze and again at launch load.

Constraining an input and templating the reach is what makes a plan retargetable. A single frozen and published plan is pointed at any admitted endpoint without a re-freeze, because the signature covers the reach template and the constraint that bounds it rather than one fixed endpoint.
