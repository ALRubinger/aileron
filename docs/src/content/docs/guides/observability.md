...
| `aileron.reach.enforced` | Whether the reach was enforced. `true` when the hosts are the verified lock's sealed `stepTrust` and the step ran under a step-scoped proxy credential restricted to exactly them. |
