...
You ran a real agent against a real LLM. Your installed actions reached the agent through Aileron's MCP server (one tool per action, description drawn from the action's Markdown body). Installed frozen Flight Plans reach the agent the same way (one tool per plan, with an input schema derived from the plan's declared inputs); calling one launches the deterministic plan on the daemon. When the LLM picked one, the agent host dispatched the call back through MCP to the daemon, which executed the action inside a WASM sandbox, mediated OAuth tokens at the network boundary so the LLM never held a credential, and recorded every decision and execution in an audit log.
