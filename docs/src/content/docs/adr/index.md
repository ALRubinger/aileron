...
- [ADR-0008: Action Exposure to Agents](/adr/0008-intent-matching) — MCP is the current host-launch action exposure surface; v4 sandbox launch supersedes that runtime shape with generated HTTPS shims and the data-plane direction in ADR-0018 through ADR-0020.
...
- [ADR-0018: v4 Single-Binary Runtime Model](/adr/0018-v4-single-binary-runtime) — *Proposed.* v4 keeps one `aileron` runtime model with multiple modes/helpers. The sandbox runtime does not revive `aileron-mcp` as an in-container sidecar or MCP-server architecture.
...
- [ADR-0021: v4 Shell-Layer Mediation](/adr/0021-v4-shell-layer-mediation) — *Proposed.* Inside the sandbox container, Aileron can own bash and mediate shell commands through a wrapper, DEBUG trap, and always-async approval semantics.
