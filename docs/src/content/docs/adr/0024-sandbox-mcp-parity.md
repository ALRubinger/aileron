...
- **Daemon network exposure.** Host launch binds loopback only. To be reachable from the container via `host.docker.internal`, the daemon binds either an explicit `host-gateway` interface (`--add-host=host.docker.internal:host-gateway` since Docker 20.10) or a non-loopback address. Any process on the host with `AILERON_TOKEN` and reachability to the same port gains the same authority `aileron-mcp` does. The token authority is the only post-mount credential. Token-scoping (per-session token issuance / per-route caveat) is not introduced in this PR; it is a candidate follow-up.
...
- **Image-bake for sealed runtimes.** See "Alternatives Considered" → image-bake deferred.
