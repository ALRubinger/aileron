...
- The runtime maintains a per-platform "sandbox available" probe, run at startup and at install of spawn-using connectors. The result feeds the structured `spawn_sandbox_unavailable` error. The probe runs at `aileron action install` time so the user sees unavailability before any binary is fetched or stored.
