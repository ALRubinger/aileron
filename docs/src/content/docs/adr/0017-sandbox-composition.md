...
Composing standard `features` requires either the official `@devcontainers/cli` or a reimplementation of Feature resolution and ordering. The final call is to drive sandbox builds through `@devcontainers/cli` so standard `features` compose without Aileron reimplementing the resolver. The alternative considered is a raw `docker build` that runs each Feature's install script in order; it is recorded as the fallback if the CLI dependency proves unworkable.
