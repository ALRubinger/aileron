...
- **`client_secret` is optional and treated as a public client identifier.** Some providers (Google's "Desktop app" OAuth client type, notably) reject token-exchange requests that omit a registered `client_secret` even when PKCE is used. The publisher MAY set `client_secret` in the connector manifest's `[capabilities.credential.oauth2]` block; the runtime forwards it on token exchange and refresh when present. The "secret" ships in the connector binary — the same way `gcloud`, `gh`, and every distributed installed-app client embeds it — and is not cryptographically secret in this flow per the providers' own installed-app guidance. PKCE remains what binds an authorization code to the session that started it; the `client_secret` (when required) is provider plumbing, not a session credential. Providers that genuinely treat `client_secret` as a server-only credential (web-app flows behind a hosted backend) MUST NOT have it set in a connector manifest.
