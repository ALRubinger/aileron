...
A frozen Flight Plan is portable across machines. Freeze stays local, hermetic, and offline, and a separate `skill publish` verb performs the network-touching act of pushing the frozen version to an OCI registry. Publish pushes one OCI reference that carries both the composed image and the signed artifact, where the signed artifact bundles the `SKILL.md`, the `aileron.lock`, the detached signature, and the publisher's public signing key as an OCI referrers manifest. A second operator runs `skill install <oci-ref>` to pull that reference and verify it, and no re-freeze happens on the second machine. The composed image is pinned by a verifiable digest, so the install-by-reference path resolves and verifies the exact image the publisher sealed.

The image digest a composed pin attests is a serialization-agnostic config content digest. Freeze parses the composed image config and hashes its execution-relevant fields. Those fields are the root filesystem layer identities (`rootfs.diff_ids`) plus the runtime config (`Env`, `Entrypoint`, `Cmd`, `User`, `WorkingDir`, `Volumes`, `ExposedPorts`, `Labels`, `StopSignal`) and the platform (`os`/`architecture`). Serialization-only fields are dropped before hashing, so the container build history and the config blob encoding do not participate in the identity. Publish re-checks this content digest on the local image before pushing and on the pushed image after the push, and install re-checks it against the same signed value on the second machine. A composed pin therefore binds by content digest, and an image-only or custom-base pin still binds by the base image's registry manifest digest.

A composed pin carries this content digest as a per-arch set, not a single value. The signed lock records a `configDigests` set with one entry per platform the composed image was built for, keyed by `(os, arch)`. The operating system is always `linux` for a container image, so the platform key is effectively the architecture. Launch, publish, and install each select the entry for the host platform (`linux` plus the host `runtime.GOARCH`) and verify against it, and a plan that carries no entry for the host platform fails closed rather than booting an unattested image. There is no aggregate index digest across the per-arch entries: the manifest-index digest is unstable across the config-blob re-serialization a registry performs on push, so binding to it would defeat the same content-stability property the per-arch content digest provides (this aggregate-digest binding was considered and rejected under #2014). The set replaces the earlier single composed-pin digest field; a foreign-base pin keeps its single manifest `digest`.

Freeze builds the composed image twice: once as a multi-arch OCI layout whose per-arch config content digests become the `configDigests` set above, and once as a separate host-architecture image loaded into the local daemon so an unpublished plan launches straight from the daemon with no publish step in between. The two builds use different buildkit drivers with separate layer caches over composed layers that are not bit-reproducible, so their `rootfs.diff_ids` and therefore their config content digests can legitimately differ. `configDigests` is the published identity that publish and cross-machine install verify. The local no-publish boot compares against a separate `localHostConfigDigest` field that freeze records as the daemon-loaded host image's own config content digest, so freeze-then-launch on the same machine is self-consistent by construction rather than mis-refusing the operator's own freshly frozen image (#2138). A lock frozen before this field falls back to the host `configDigests` entry for the local boot compare.

Binding to the content digest rather than to the raw config blob's own hash is what keeps publish correct under Docker's containerd image store. That store re-serializes the image config on `docker push`, which changes the config blob's own hash while leaving every execution-relevant field byte-identical. The raw-blob check read that benign re-encode as tampering and refused a valid publish. The content digest is stable across the re-encode, so the same image verifies on either image store. The integrity guarantee is preserved because every field that changes what the container executes is still covered, so a substituted entrypoint, environment, or filesystem layer changes the digest and fails the check.
