...
- Rung-three dispatch hardening. Freeze pins the rung-three per-step sibling images by digest and the launch-time runtime now mounts each image, runs it, and collects its output as the step's dataflow. A rung-three step's network reach is now declared in its per-step trust contract and sealed onto its lock pin, so the reach is covered by the signature and not re-suppliable at launch. What remains a follow-up is consuming that sealed reach and the egress hardening around it. Stamping the sealed reach onto the launch host allow-list is the next step (#1769). The fail-closed behavior when a step declares no reach is a separate follow-up (#1758). The per-step egress firewall that enforces the reach at the network boundary is its own follow-up (#1736).
