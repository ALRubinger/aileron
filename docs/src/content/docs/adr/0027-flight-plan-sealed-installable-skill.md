...
- Rung-three dispatch hardening. Freeze pins the rung-three per-step sibling images by digest and the launch-time runtime now mounts each image, runs it, and collects its output as the step's dataflow. What remains a follow-up is the credential and egress hardening of that dispatch: the tool image is credential-free and its egress is mediated by the data plane, and the audit hardening and egress policy for per-step dispatch are separate follow-ups.
