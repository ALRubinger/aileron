...
**Flight-Plan publishers.** Keyring v2 also covers Flight-Plan publishers. A Flight Plan frozen with `aileron skill freeze --publisher <authority>` records the publisher inside its signed lock, and `aileron skill launch` resolves the plan's signing key against the same owner-level and per-repo union this section defines. The union resolution and the fail-closed-on-empty rule are identical to connector verification, so `aileron keyring trust <publisher>` trusts that publisher's connectors and their Flight Plans at once. Enforcement is launch-only. Install handles unsigned pre-freeze bytes that carry no publisher lock to verify, so install gating stays out of scope. The plan-side divergence signal is distinct from the connector `trust_state: conflict` above. The connector conflict fires when a fetched key is not in the trusted owner set at preview. The plan-side conflict fires when the owner-level and per-repo scopes both trust keys, the sets differ, and the plan's signing key is a member of the union, in which case the launch is permitted and the divergence is surfaced as a diagnostic. A plan signing key that is not in the union at the launch gate is simply refused.
