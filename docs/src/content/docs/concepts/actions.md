...
Aileron exposes your installed actions to the agent through MCP. The Aileron MCP server (`aileron-mcp`) advertises one tool per installed action — name, description (the action's Markdown body), and parameter schema (derived from the manifest's `[[inputs]]` block). The agent host merges Aileron's tools with whatever other tools it has; the LLM picks among the merged set as it would for any tool. When it picks an Aileron-installed action, the agent host dispatches the call back to `aileron-mcp`, which forwards it to the daemon's `POST /v1/actions/{name}/run` endpoint. The daemon executes the action server-side, honors the manifest's `[approval]` block, and returns the result. See [Deterministic Agentic Execution](/concepts/deterministic-agentic-execution/) for how MCP composes with the agent's existing tool catalog, and [ADR-0008: Action Exposure to Agents](/adr/0008-intent-matching) for why MCP is the only path.

The agent doesn't need to know that the tool came from Aileron — to its host, it's just another MCP tool. The execution layer is on the daemon side: the agent has no way to bypass the approval gate, no flag to flip, no SDK to swap. There is exactly one execution path for every action invocation, and it's the place where capability checks, credential binding, audit emission, and approval gating happen.
