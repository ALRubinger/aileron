...
| `aileron status [runtime\|notifications\|vault\|host-bindings]` | Report the running runtime: version, listen address, action count, connector count, binding count, vault state. The optional section argument narrows output to one of `runtime`, `notifications`, `vault`, or `host-bindings`. The `host-bindings` section reads the merged host-binding descriptor table locally (no daemon) and reports a broken descriptor (e.g. an un-substituted `<...>` placeholder) before launch. Read-only; safe to run frequently. |
...
| `aileron keyring trust --plan <name>[@<version>]` | Trust a locally-frozen plan's OWN signing key at its declared per-repo publisher authority, with no network fetch. Use this to launch a plan you just froze or published whose signing key is not the repository's committed `keys/publisher.pub` (the `keyring trust <publisher>` form fetches that committed key, a different one, and no-ops). The key is read from the plan's `signing-key.pub` in the local store; the authority is read from the plan's signed lock. Grants per-repo scope only, so it never widens owner-level trust. |
