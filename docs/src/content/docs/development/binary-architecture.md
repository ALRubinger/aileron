...
description: "The three binaries that make up an Aileron install, the trust boundary each owns, and how they connect to the agent host."
order: 2
---

An Aileron install ships as three cooperating binaries. Each owns a narrow trust boundary; the install pipeline puts them in your PATH and the agent host calls into whichever one it needs. End users rarely think about more than `aileron` itself, but every page in this docs section assumes you understand the split.

## The three binaries

| Binary | Source | Trust boundary | Called by |
|---|---|---|---|
| `aileron` | [`cmd/aileron`](https://github.com/ALRubinger/aileron/tree/main/cmd/aileron) | CLI plus user-scoped local daemon (per [ADR-0012](/adr/0012-local-daemon-architecture)). The vault, sessions, audit log, connector store, and binding store all live behind the daemon. | The user (interactive CLI) and the other binaries (as clients). |
| `aileron-mcp` | [`cmd/aileron-mcp`](https://github.com/ALRubinger/aileron/tree/main/cmd/aileron-mcp) | MCP server adapter. Translates an MCP `tools/call` request from the agent host into an HTTP POST to the local daemon's `/v1/actions/{name}/run`. | The agent host (Claude Code, Codex CLI, Goose, OpenCode, anything that speaks MCP). |
...
Per [ADR-0015](/adr/0015-launch-audit-scope), Aileron's audit boundary is the actions Aileron executes (connector installs, action invocations, binding lifecycle, gateway-routed LLM requests). Commands the agent runs locally through its own exec tool — `git`, `go test`, `rm` — are outside this boundary; the agent's own approval / sandbox layer mediates those.
...
All three binaries are built from the same repo at the same commit. The release pipeline produces a matched set; a `task build` builds all three. Mismatched versions (e.g., a newer `aileron-mcp` talking to an older daemon) are not supported, because the daemon's HTTP API is the contract and we have not committed to compatibility across versions yet.
