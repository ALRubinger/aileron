...
This section preserves the architecture decision records (ADRs `0001` through `0020`) that captured Aileron's design before the [strategic pivot](/) to a deterministic execution layer for AI agents. They remain here as immutable historical record.
...
Per the project's ADR immutability rule, none of these are edited. Instead, the live [`Architecture Decisions`](/architecture/) section (currently empty) is being repopulated with fresh post-Pivot ADRs that ratify the commitments stated in the Pivot architecture pages. That work is tracked in [issue #343](https://github.com/ALRubinger/aileron/issues/343); the archival itself is tracked in issue #335 (Phase 5).
...
- **As input for the new ADRs:** the post-Pivot ADRs (in [`/adr/`](/architecture/) once they land) reference these archives where the new decision draws on, supersedes, or refutes a legacy one.
- **Not as current architecture:** if you're trying to understand how Aileron works today, start at [Pivot Overview](/), not here.
