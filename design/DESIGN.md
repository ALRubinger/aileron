...
Caveats: descender clipping is avoided by setting H1 line-height to `1.1` (not `1.0`), giving glyphs like `g`, `y`, `p` room to render fully.
...
In the docs site, **every `h2` in MDX content opens a section card**. A rehype plugin (`rehype-section-wrapper`) walks the rendered tree and wraps each `h2` plus its following siblings (until the next `h2`) in a `<section class="prose-section">` element. The CSS for `.prose-section` gives that wrapper a white card surface, `--shadow-low`, and rounded corners. Authors write flat markdown; the section chrome is added at build time.
