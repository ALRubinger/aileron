...
// the immediately-preceding label. It does not require a ".amazonaws.com"
// suffix, so test endpoints like "athena.us-east-1.amazonaws.test" parse
// the same way as production hosts.
//
...
func ParseAWSEndpointHost(host string) (service, region string, err error) {
	...
	for i, label := range labels {
		...
		if i == 0 {
			return "", "", fmt.Errorf(
				"%w: %q has a region-shaped leading label with no service label",
				ErrUnparseableAWSHost, host)
		}
		return labels[i-1], label, nil
	}
