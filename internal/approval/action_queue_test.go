...
// TestActionApprovalQueue_RegisterShellSetsKindAndArgs asserts the
// shell-shim entry point (#427) stamps Kind = "shell" with the
// command/reason/cwd packed into Args. The webapp's shell-card
// layout reads these by key; if a refactor changes the field shape,
// this test is the one that should break.
func TestActionApprovalQueue_RegisterShellSetsKindAndArgs(t *testing.T) {
	q := NewActionApprovalQueue(nil, nil)
	a := q.RegisterShell("git push origin main", "command requires approval", "sess-9", "/home/me/project")

	if a.Kind != ApprovalKindShell {
		t.Errorf("RegisterShell Kind = %q, want %q", a.Kind, ApprovalKindShell)
	}
	if a.ActionName != "shell" {
		t.Errorf("RegisterShell ActionName = %q, want %q", a.ActionName, "shell")
	}
	if a.ConnectorFQN != "" {
		t.Errorf("RegisterShell ConnectorFQN = %q, want empty", a.ConnectorFQN)
	}
	if got, want := a.Args["command"], "git push origin main"; got != want {
		t.Errorf("Args[command] = %v, want %q", got, want)
	}
	if got, want := a.Args["reason"], "command requires approval"; got != want {
		t.Errorf("Args[reason] = %v, want %q", got, want)
	}
	if got, want := a.Args["cwd"], "/home/me/project"; got != want {
		t.Errorf("Args[cwd] = %v, want %q", got, want)
	}
	if a.SessionID != "sess-9" {
		t.Errorf("SessionID = %q, want %q", a.SessionID, "sess-9")
	}
}

// TestActionApprovalQueue_RegisterShellOmitsCwdWhenEmpty asserts
// that Args["cwd"] is absent rather than present-as-empty when the
// caller doesn't supply one. The webapp renders "in <cwd>" only when
// the key is present; an empty value would render an awkward "in ".
func TestActionApprovalQueue_RegisterShellOmitsCwdWhenEmpty(t *testing.T) {
	q := NewActionApprovalQueue(nil, nil)
	a := q.RegisterShell("ls", "ask rule", "sess-1", "")

	if _, ok := a.Args["cwd"]; ok {
		t.Errorf("Args[cwd] present, want absent for empty cwd")
	}
}

// TestActionApprovalQueue_DecideForwardsEditedPayloadForShell pins
// the "ride save_policy through edited_payload" convention that the
// launch-side approval socket depends on. Decide forwards the map
// verbatim onto ActionDecision.EditedPayload; the socket reader
// translates from there into the wire string aileron-sh expects.
func TestActionApprovalQueue_DecideForwardsEditedPayloadForShell(t *testing.T) {
	q := NewActionApprovalQueue(nil, nil)
	a := q.RegisterShell("git push", "ask rule", "sess-1", "")

	done := make(chan ActionDecision, 1)
	go func() {
		d, _ := a.Wait(context.Background(), time.Second)
		done <- d
	}()

	if err := q.Decide(a.ID, true, "", map[string]any{"save_policy": "project"}); err != nil {
		t.Fatalf("Decide: %v", err)
	}
	got := <-done
	if !got.Approved {
	...
	if got.EditedPayload["save_policy"] != "project" {
		t.Errorf("EditedPayload[save_policy] = %v, want %q", got.EditedPayload["save_policy"], "project")
	}
}
