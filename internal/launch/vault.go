...
// Pre-ADR-0012 this file also held resolveVaultPromptMode and
// vaultPathForGateway, which selected between stderr-prompt and
// webapp-modal vault unlock for the embedded gateway. ADR-0012 moves
// vault unlock entirely to the daemon (`POST /v1/vault/local/unlock`),
// so launch no longer chooses a prompt surface — the daemon's status
// endpoint is the source of truth, and the webapp modal is the user's
// unlock surface. EnsureVault stays because the daemon binary
// (internal/server) still uses it for stderr-prompt unlock at startup.
